...
if "function StripBanners" not in code:
    code = code.replace("export default function HomePage", strip_banner_component + "\\nexport default function HomePage")

new_strip_block = """        {/* STRIP BANNER */}
        <StripBanners strips={banners.filter(b => b.type === "STRIP")} />"""

# Find the old strip banner block
pattern = r'\{\/\* STRIP BANNER \*\/\}\s*\{banners\.filter[\s\S]*?\)\}'
code = re.sub(pattern, new_strip_block, code)
