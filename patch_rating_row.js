...
const oldRatingRowRegex = /const RatingRow = \(\{.*?\}\);\s*\};\s*};/s;
...
code = code.replace(oldRatingRowRegex, newRatingRow);

const oldTextareaRegex = /<div className="mt-4 flex-1 flex flex-col min-h-\[150px\]">.*?<\/textarea>\s*<\/div>/s;
const newTextarea = `<div className="mt-4">
...
code = code.replace(oldTextareaRegex, newTextarea);

fs.writeFileSync(file, code);
console.log('Successfully patched RatingRow and Textarea in FeedbackModal.');
