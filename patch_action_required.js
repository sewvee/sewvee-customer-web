...
const target = `  if (msgText && msgText.startsWith("Category:")) {`;

const newCode = `  // Parse Action Required Messages
...
if (code.includes(target) && !code.includes("[ACTION_REQUIRED: PHOTO_REQUEST]")) {
  code = code.replace(target, newCode);
  fs.writeFileSync(file, code);
  console.log('Successfully added styling for [ACTION_REQUIRED: PHOTO_REQUEST]!');
} else {
  console.log('Failed to patch ACTION_REQUIRED.');
}
