...
// 2. Add State variables and handleConfirmOutfitPhotos
content = content.replace(
  ...
  const handleConfirmOutfitPhotos = async () => {
    ...
    try {
      ...
      for (const u of urls) {
        await fetch(\`\${URL_CUSTOMER_PORTAL_ORDERS}/\${order.id.toString()}/requests/\${oId}\`, {
          method: 'POST',
...
const oldButtonRegex = /onClick=\{\s*async\s*\(\)\s*=>\s*\{\s*const\s+oId\s*=\s*activeOutfit\.id[^]*?setSubmittingOutfitId\(null\);\s*\}\s*\}\s*\}/m;
